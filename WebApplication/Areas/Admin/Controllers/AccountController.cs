...
namespace WebApplication.Areas.Admin.Controllers
{
    ...
    public class AccountController : BaseController
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var checkConfirm = db.AspNetUsers.FirstOrDefault(c => c.UserName == model.UserName);
            if(checkConfirm == null)
            {
                ModelState.AddModelError("", "Invalid login attempt.");
                return View(model);
            }
            if(checkConfirm != null && checkConfirm.StatusConfirm == 777)
            {
                ModelState.AddModelError("", "Invalid login attempt.");
                return View(model);
            }
        ...
        public ActionResult Register()
        {
            ...
            {
                TempData["role"] = context.Roles.ToList();
            }
            else
            {
                TempData["role"] = context.Roles.Where(a => a.Id == "KTV").ToList();
            }
            return View();
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase ImgIDFront, HttpPostedFileBase ImgIDBack)
        {
            if (ModelState.IsValid)
            {
                if (Utils.Control.getMobileOperator(model.PhoneNumber) == "UNKNOWN")
                {
                ...
                if (result.Succeeded)
                {
                    ...
                    _user.Ward = model.Ward;
                    _user.District = model.District;
                    _user.Province = model.Province;
                    _user.Createby = User.Identity.Name;
                    _user.Createdate = DateTime.Now;
                    _user.FullName = model.FullName;
                    _user.StationHead = model.StationHead;
                    _user.Fee = model.Fee;
                    _user.Company = model.Company;
                    _user.Tax = model.Tax;
                    _user.Bank = model.Bank;
                    _user.BankNumber = model.BankNumber;
                    _user.InvoiceType = model.InvoiceType;
                    if(model.Role == "KTV")
                    {
                        ...
                        _user.ImgIDFront = linkImgFr;
                        _user.ImgIDBack = linkImgBa;
                        //trạng thái chờ duyệt tài khoản set là 777
                        _user.StatusConfirm = 777;
                    }
            ...
            SetAlert("Tạo tài khoản không thành công", "danger");
            return RedirectToAction("Index", "User");
        }
